...
START_TEST(test_one) {
  ...
  const char math_expression[100] = "2mod3";
...
START_TEST(test_two) {
  ...
  const char math_expression[100] = "3mod2";
  char *result = malloc(50 * sizeof(char));
  result = execution(math_expression);
...
Suite *suite_smart_calc_mod(void) {
  Suite *s = suite_create("suite_smart_calc_mod");
  TCase *tc = tcase_create("case_mart_calc_mod");

...
START_TEST(test_one) {
  char answer[64] = "1.4142136"; 
  const char math_expression[100] = "sqrt2";
...
START_TEST(test_two) {
  ...
  const char math_expression[100] = "sqrt1";
  char *result = malloc(50 * sizeof(char));
  result = execution(math_expression);
...
Suite *suite_smart_calc_sqrt(void) {
  Suite *s = suite_create("suite_smart_calc_sqrt");
  TCase *tc = tcase_create("case_mart_calc_sqrt");

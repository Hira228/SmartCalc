...
START_TEST(test_one) {
  char answer[64] = "0.8414710"; 
  const char math_expression[100] = "sin1";
...
START_TEST(test_two) {
  ...
  const char math_expression[100] = "sin3.1415927";
  char *result = malloc(50 * sizeof(char));
  result = execution(math_expression);
...
Suite *suite_smart_calc_sin(void) {
  Suite *s = suite_create("suite_smart_calc_sin");
  TCase *tc = tcase_create("case_mart_calc_sin");

...
char *execution(const char *str) {
  ...
  {
    flag_error_clear(&stack, &reverse_stack, &stack_polish_notation,
                     &stack_reverse_polish_notation, &stack_calc);
  ...
  flag_error =
      reverse_polish_notation(&reverse_stack, &stack_polish_notation);  // RPN
  ...
  reverse_stack_elements(&stack_polish_notation,
                         &stack_reverse_polish_notation);  // REVERSE STACK

  flag_error =
      calculate(&stack_reverse_polish_notation, &stack_calc);  // CALCULATE
...
int calculate(struct Data **stack_reverse_polish_notation,
  ...
  while (!stack_is_empty(*stack_reverse_polish_notation) &&
    ...
    if (temp_n->type == NUMBER)
      push_back(stack_calc, temp_n->value, temp_n->type, temp_n->priority);
    else {
      double a = 0;
      char numb[64] = {0};
      ...
      switch (temp_n->type) {
        ...
        case DIV:
          if (stack_is_empty(*stack_calc) || a == 0.0f)
            flag_error = FAILURE;
          else
            push_back(stack_calc, pop_back_val(stack_calc) / a, NUMBER, 0);
          break;
        ...
        case POW:
          if (stack_is_empty(*stack_calc))
            ...
            push_back(stack_calc, powf(pop_back_val(stack_calc), a), NUMBER, 0);
          break;
        ...
        case SIN:
          if (fabs(sin(a) - (-0.0f)) < 1e-6) {
            push_back(stack_calc, 0.0f, NUMBER, 0);
          } else if (flag_error == SUCCESS) {
            push_back(stack_calc, sin(a), NUMBER, 0);
          }
          break;

        case COS:
          if (fabs(cos(a) - (-0.0f)) < 1e-6) {
            push_back(stack_calc, 0.0f, NUMBER, 0);
          } else if (flag_error == SUCCESS)
            push_back(stack_calc, cos(a), NUMBER, 0);
          break;
        case SQRT:
          if (flag_error != FAILURE && a >= 0.0f)
...
int check_unary_minus(struct Data *ptr) {
  ...
  if (!stack_is_empty(ptr)) temp = ptr->next;
...
int reverse_polish_notation(struct Data **reverse_stack,
  ...
  while (!stack_is_empty(*reverse_stack) && flag_error == SUCCESS) {
    ...
    if (temp->type == NUMBER) {
      push_back(
          stack_polish_notation,
          (flag_unary == UNARY_MINUS ? (temp->value) * (-1) : (temp->value)),
          temp->type, temp->priority);

      if (flag_unary == UNARY_MINUS) flag_unary = SUCCESS;
    }

    else if (temp->type != NUMBER && temp->type != PRNTS_OPEN &&
             temp->type != PRNTS_CLOSE) {
      while (!stack_is_empty(stack_operator) &&
             peek_stack_priority(stack_operator) >= temp->priority &&
             stack_operator->type != PRNTS_OPEN) {
        struct Data *temp_op = stack_operator;
        push_back(stack_polish_notation, temp_op->value, temp_op->type,
                  temp_op->priority);
      ...
      if (!stack_is_empty(stack_operator) &&
          stack_operator->type == PRNTS_OPEN) {
...
int stack_is_empty(struct Data *ptr) { return ptr == NULL; }

int peek_stack_priority(struct Data *ptr) { return ptr->priority; }

int peek_stack_op(struct Data *ptr) { return ptr->type; }
